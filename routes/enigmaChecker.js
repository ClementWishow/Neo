...
function checkParticularity(stepData, req, ret) {
    let prompt = req.body.prompt ? req.body.prompt.toLowerCase() : null
    switch (stepData.name) {
        ...
        case "yellow":
            if (ret.correct && req.body.styles) {
                ret.correct = req.body.styles.color.replace(/\s/g, '') === "rgb(255,255,0)"
                if (!ret.correct && req.body.styles.color.replace(/\s/g, '') != "rgb(51,208,17)") {
        ...
        case "end":
            ret.correct = true
            let cookieStep = req.cookies['x-key'] ? data.findIndex(elem => elem.key === req.cookies['x-key']) : 0
...
exports.checkEnigma = function (req) {
    ...
    if (!ret.redirect) {
        ret.redirect = ret.correct ? req.app.locals.baseURL + (step + 1).toString() : null
