    ...
    case "endform":
      ...
      if (req.body.try === 1) {
        ...
        user = await findUserById(
          Buffer.from(decrypt(req.cookies["x-id"]).toString("hex"))
        );
      }
      switch (req.body.try) {
        case 2:
      ...
      ret.message =
        stepData.alternateAnswer[
          Math.min(stepData.alternateAnswer.length - 1, req.body.try - 1)
        ];
...
export async function checkEnigma(name, req) {
  ...
  const answerToLowerCase = stepData.answer.map((answer) =>
    answer.toLowerCase()
  );
  ret.correct = answerToLowerCase.includes(prompt);
  ...
  if (!ret.message && !req.body.mutation) {
    ...
    if (!ret.correct && stepData.otherWrongs && req.body.try > 1) {
      ret.message =
        stepData.otherWrongs[
          Math.min(stepData.otherWrongs.length - 1, req.body.try - 2)
        ];
    }
  }
  return ret;
