...
var observer = new MutationObserver(function(mutations) {
	mutations.forEach(function(mutation) { 
		if (mutation.target && (mutation.target.className.includes('typewriter-item') || mutation.target.id === "typeWriter")) {
			return
		}
		const target = mutation.target ? 
		{"class": mutation.target.className, 
		"id" : mutation.target.id,
		"name": mutation.target.nodeName} : null
		const added = Array.from(mutation.addedNodes).map(x => ({"class": x.className, "id" : x.id, "name": x.nodeName}))
		const deleted = Array.from(mutation.removedNodes).map(x => ({"class": x.className, "id" : x.id, "name": x.nodeName}))
